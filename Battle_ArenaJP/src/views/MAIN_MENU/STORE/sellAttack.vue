    ...
    async fetchAttacks() {
      try {
        ...
        if (response.headers['content-type'].includes('application/json')) {
          ...
          this.attacks = response.data.filter((attack) => !attack.equipped);
        } else {
          console.error('Error: The response does not contain JSON data');
        }
      } catch (error) {
    ...
    async sellSelectedAttack() {
      try {
        ...
        const response = await axios.post(
          `https://balandrau.salle.url.edu/i3/shop/${this.selectedAttack.attack_ID}/sell`,
        ...
        if (response.status === 200) {
          console.log('Attack sold:', response.data);
          this.fetchAttacks();
          this.closeSellPopup();
        } else {
